...
import { getCompetitionByYear, getScorersByCompetition } from '@/redux/slices/Competitions';
...
export const useDateFilterHook = (isSeason: boolean): IUseDateFilterHookReturn => {
  ...
  // считает календарный год надо исправить на турнирный
  ...
  const handleYear =
    ...
    (e) => {
      ...
      const date = currentTarget.getAttribute('data-date') as string;
      setSelected(date);
      dispatch(getScorersByCompetition({ id, date }));
